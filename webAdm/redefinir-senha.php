<?php
/**
 * Página pública — conclusão do Fluxo 2 (recuperação de senha). Standalone,
 * fora do fluxo autenticado, mesmo padrão de ativar-conta.php/responder.php.
 */
...
if ($token === '') {
    $erroToken = 'Link de redefinição inválido. Verifique se você copiou o endereço completo enviado por e-mail.';
} elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $senha = $_POST['senha'] ?? '';
    $confirmacaoSenha = $_POST['confirmacao_senha'] ?? '';

    $resp = $api->post('auth/resetar-senha', [
        'token'             => $token,
        'senha'             => $senha,
    ...
    if ($resp['success']) {
        ...
        $erroFormulario = $resp['data']['mensagem'] ?? 'Não foi possível redefinir a senha. Tente novamente.';
    ...
    $respValidacao = $api->get("auth/resetar-senha/{$token}");
    if ($respValidacao['success']) {
        $nomeUsuario = $respValidacao['data']['dados']['nome'] ?? null;
    } else {
        $erroToken = $respValidacao['data']['mensagem'] ?? 'Este link de recuperação não é mais válido.';
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>busola — Redefinir senha</title>
...
<div class="wrap">
  <div class="card-auth">
    ...
    <?php if ($concluido): ?>
      <h1><i class="bi bi-check-circle-fill text-success me-1"></i> Senha redefinida!</h1>
      <p class="subtitle">Sua nova senha já está ativa. Você já pode entrar na plataforma.</p>
    ...
    <?php elseif ($erroToken): ?>
      ...
      <p class="subtitle mb-0">Solicite uma nova recuperação de senha na tela de login.</p>
      <a href="login.php" class="voltar-login">&larr; Voltar para o login</a>

    <?php else: ?>
      <h1>Redefinir senha</h1>
      <p class="subtitle">
        Olá<?php echo $nomeUsuario ? ', ' . htmlspecialchars($nomeUsuario) : ''; ?>! Escolha sua nova senha de acesso.
      </p>
      ...
      <form method="POST" action="redefinir-senha.php?token=<?php echo urlencode($token); ?>" id="formRedefinir">
        <div class="mb-3">
        ...
        <button type="submit" class="btn-acao"><i class="bi bi-shield-check me-1"></i> Redefinir minha senha</button>
      </form>
    <?php endif; ?>
  </div>
</div>
...
<script>
  const forms = document.getElementById('formRedefinir');
  if (forms) {
